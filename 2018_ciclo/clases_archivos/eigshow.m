...
uicontrol(...
    ...
    'string',getString(message('MATLAB:demos:eigshow:ButtonHelp')), ...
    'callback','helpwin eigshow')
uicontrol(...
    'style','pushbutton', ...
    'units','normalized', ...
    'position',[.86 .40 .12 .06], ...
    'string',getString(message('MATLAB:demos:eigshow:ButtonClose')), ...
    'callback','close(gcf)')
uicontrol(...
    'style','popup', ...
    'units','normalized', ...
...
h.text = text(v(1)/2,v(2)/2,t,'fontsize',12,'color',color);
h.anim = animatedline('LineStyle','none','marker','.','color',color);
